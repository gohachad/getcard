...
export const ru = {
  meta: {
    ...
    description:
      'Карта VISA от ведущего киргизского банка. Два паспорта — и карта ваша. Оплата Netflix, Spotify, ChatGPT, Claude, Steam, App Store и покупок за границей. Доллары, евро, дирхамы и рубли. Работает онлайн ещё до доставки пластика.',
  ...
  process: {
    ...
    steps: [
      {
        title: 'Вы присылаете сканы паспортов',
        body: 'Внутренний и заграничный паспорт — сканы, не фотографии. Займёт десять минут.',
      },
      {
    ...
    items: [
      {
        ...
        a: 'Мы принимаем паспорта любых стран, не только российские. Нужен внутренний паспорт и заграничный. Окончательное решение по каждой заявке принимает банк.',
      },
      {
        q: 'Сколько документов нужно?',
        a: 'Два паспорта, три скана. Внутренний паспорт — разворот с фотографией и страница с пропиской. Заграничный паспорт — страница с фотографией. Нужны именно сканы, а не снимки на телефон: банк не принимает фотографии. Ещё короткая форма с контактами и профессией. Справки о доходах, подтверждение адреса и нотариальные документы не требуются.',
      },
      {
  ...
  footer: {
    ...
    legalLines: [
      ...
      'Мы принимаем паспорта любых стран. Нужна одна личная встреча в Москве, остальные этапы проходят онлайн.',
